...
export async function history(channelId: string, opts: { json: boolean; top?: number; afterTs?: string; content?: boolean }) {
  ...
  const messagesRequest = opts.afterTs
    ? paginate("conversations.history", { channel: resolvedId, limit, oldest: opts.afterTs, inclusive: false }, "messages")
    : call("conversations.history", { channel: resolvedId, limit }).then(result => result.messages || []);
  const [raw, info] = await Promise.all([
    messagesRequest,
    call("conversations.info", { channel: resolvedId }).catch(() => ({ channel: {} })),
  ]);
  const selected = raw
    .filter((message: any) => !opts.afterTs || Number(message.ts) > Number(opts.afterTs))
    .sort((a: any, b: any) => Number(a.ts) - Number(b.ts))
    .slice(-(opts.top ?? 20));
  const normalized: ReturnType<typeof normalizeMessage>[] = selected.map((message: any) => normalizeMessage(message));
  ...
  const result = { channel: info.channel?.name || info.channel?.user || channel.label, channel_id: resolvedId, messages };
