...
const flagSpecs = {
  ...
  params: true, "unsafe-method": false,
} as const;
...
function validateInvocation() {
  ...
  switch (command) {
    ...
    case "send": validateFlags(["json", "allow-write", ...shared]); return requirePositionals(2, Infinity, "slack-cli send <channelId> <text> --allow-write");
...
Write (each requires --allow-write):
  send <channelId> <text>                        Post a message
  mark <channel> [ts]                             Mark through ts, or through now when ts is omitted
  api <write-method> --params JSON                Call a catalogued write method
...
Common flags:
  ...
  --allow-write                                   required for every mutation

Use --help for this summary. Use \`api methods\` or \`api describe <method>\`
before calling the direct API; unknown methods require --unsafe-method and --allow-write.`);
}
